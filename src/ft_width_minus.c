...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/23 22:09:53 by lgaultie          #+#    #+#             */
/*   Updated: 2019/04/23 22:10:10 by lgaultie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char		*ft_width_sup_conv_sz(int width, int i, int surplus, t_data *d)
{
	char	*ret;
...
static char		*ft_width_minus2(int width, t_data *d)
{
	...
	if (width > d->conv_sz)
	{
		if (!(ret = ft_width_sup_conv_sz(width, i, surplus, d)))
			return (NULL);
	}
	else
		return (ft_strdup(""));
